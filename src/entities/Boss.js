...
// Boss with basic phase/timer fields; attack behavior implemented in Game for flexibility
export default class Boss{
  ...
  update(dt){ if(!this.active) return; // approach
    if (this.x > 360) this.x -= this.s * dt; // move into screen
    // simple vertical oscillation for motion
    this.y += Math.sin((performance.now()/600) + this.phase) * 6 * dt;
