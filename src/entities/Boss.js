...
/**
 * Boss: skeleton with phases. Attack logic lives in Game._bossLogic to keep entity generic
 */
export default class Boss{
  ...
  update(dt){ if(!this.active) return; if (this.x > 360) this.x -= this.s * dt; this.y += Math.sin((performance.now()/600) + this.phase) * 6 * dt; this.phaseTimer += dt; this.attackTimer += dt; if (this.phaseTimer > 6){ this.phase = (this.phase + 1) % 3; this.phaseTimer = 0; } }
